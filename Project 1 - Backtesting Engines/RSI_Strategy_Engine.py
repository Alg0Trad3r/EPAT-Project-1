...
class RSI_Strategy():
  ...
  def strategy_calc_returns(self):
    ...
    self.strategy_df['Strategy_Position_Returns'] = calc_strategy_returns(close=self.strategy_df['Close'], strategy_signals=self.strategy_df['Long_Exit'], instrument_name=self.instrument_name)
  ...
  def strategy_calc_drawdown(self):
    self.strategy_drawdown = calc_drawdown(strategy_returns=self.strategy_df['Strategy_Position_Returns'], instrument_name=self.instrument_name)
  ...
  def plot_strategy_statistics(self):
    ...
    plot_returns(total_strategy_returns=self.cumulative_returns, bh_returns=self.bh_returns, instrument_name=self.instrument_name)
    ...
    plot_signals(long_signal=self.strategy_df['Long_Exit'], short_signal=self.strategy_df['Long_Exit'], instrument_name=self.instrument_name)
    ...
    plot_max_drawdown(max_drawdown=drawdown_plot, instrument_name=self.instrument_name)
  ...
  def strategy_statistics_calc(self):
    ...
    sharpe_ratio_calc(total_returns=self.strategy_df['Strategy_Position_Returns'], instrument_name=self.instrument_name)
    ...
    number_of_trades_calc(strategy_trades=self.strategy_df['Long_Exit'], strategy_signals=self.strategy_df['Long_Signal'], instrument_name=self.instrument_name)
    ...
    positive_negative_trades(strategy_returns=self.strategy_df['Strategy_Position_Returns'], instrument_name=self.instrument_name)

    # Using the function from the modular Strategy Stats file to calculate the average profit/loss
    avg_profit_loss(strategy_returns=self.strategy_df['Strategy_Position_Returns'], instrument_name=self.instrument_name)
    ...
    highest_profit_loss(strategy_returns=self.strategy_df['Strategy_Position_Returns'], instrument_name=self.instrument_name)
  ...
  def optimize_strategy_parameters(self, target_param):
        ...
        for k in self.rsi_exit_threshold:
          for l in self.stop_loss_threshold:
            for m in self.take_profit_threshold:
              ...
              elif target_param == 'Sharpe Ratio':
                self.optimize_signal_calc(i=i, j=j, k=k, l=-l, m=m)
                returns = self.strategy_calc_returns()
                self.optimize_sharpe = sharpe_ratio_calc(total_returns=self.strategy_df['Strategy_Position_Returns'], instrument_name=self.instrument_name)
              ...
              # Using the same logic that was used to optimzied for the returns to optimize
              # for the drawdown
              elif target_param == 'Drawdown':
                self.optimize_signal_calc(i=i, j=j, k=k, l=-l, m=m)
                returns = self.strategy_calc_returns()
                self.optimize_drawdown = calc_drawdown(strategy_returns=self.strategy_df['Strategy_Position_Returns'], instrument_name=self.instrument_name)
    ...
    if target_param == 'Returns':
      ...
      print(f'\n--------- {self.instrument_name} OPTIMIZED RETURNS PARAMETER VALUES ---------')
      ...
      print(f'{self.instrument_name} Best Parameters Pair: {self.best_returns_pair[0]} -> Highest Return: {self.best_returns_pair[1]}%')
    ...
    # Using the same logic that was used for the Returns to calculate for the Sharpe Ratio
      ...
      print(f'\n--------- {self.instrument_name} OPTIMIZED SHARPE PARAMETER VALUES ---------')
      # Printing the best returns and the key in the dictionary
      print(f'RSI Length: {i},\nRSI Entry Threshold: {j}, RSI Exit Threshold: {k},\nStop Loss Threshold: {-l}, Take Profit Threshold: {m}')
      print(f'{self.instrument_name} Best Parameters Pair: {self.best_sharpe_pair[0]} -> Highest Return: {self.best_sharpe_pair[1]}%')
    ...
    # Using the same logic that was used for the Returns to calculate for the Max Drawdown
      ...
      print(f'\n--------- {self.instrument_name} OPTIMIZED DRAWDOWN PARAMETER VALUES ---------')
      # Printing the best returns and the key in the dictionary
      print(f'RSI Length: {i},\nRSI Entry Threshold: {j}, RSI Exit Threshold: {k},\nStop Loss Threshold: {-l}, Take Profit Threshold: {m}')
      print(f'{self.instrument_name} Best Parameter Pair: {self.best_drawdown_pair[0]} -> Highest Return: {self.best_drawdown_pair[1]}%')
...
# Setting the instrument name to the corresponding name of the data for the instrument
# we are testing
spy_test = RSI_Strategy(data_path=data_path, instrument_name='SPY')