...
Python Version Dependencies: Python 3.12.13 or 3.12
Numpy Version Dependency: 1.26.4
Pandas Version Dependecy: 2.2.3

When setting up your vitual environment ensure that you are using these versions of Python, Numpy, and Pandas or the code will not be able to run due to a dependency and version error. Ensure to download TA-lib with pip install TA-lib into the virtual environment to be able to run the RSI Strategy Engine.

If using VSCode IDE or any IDE other than Jupyter Notebook to actually see the charts of the advanced statistics right click on the code window, go to 'Run in interative window', then choose whether you want to run the entire file or a portion of the file in that interactive window.
If it is your first time running code in an interactive window you will be prompted to download a few things before just download them and you will be able to run the code.

This is the first assigned project from QuantInsti's EPAT Program with the goal to develop a set of backtesting engines.

All of the code in this repository was written by me. No AI help other than research of specific use case syntax concepts that I then took and applied to my project.

The data that has been provided is 10 years worth of SPY data from 03/21/2016 until 03/18/2026. You will find the data I used for the project in the folder named 'Instrument Data' inside the project repository. You may need to change the path of the data being referenced to to get the code actually running.

Here is the direct prompt from QuantInsti with the requirements and goals of each individual strategy.

I. Backtests a strategy using three moving averages on any indices such as Nifty50, SPY, HSI and so on.
...
You can use the asset of your choice to backtest the strategy. You can use any source of data that you like. Some of the free data sources are as follows:

1. Pandas-datareader (Allows fetching data from Yahoo, Google and other sources)
...
6. Downloaded data to the local machine
